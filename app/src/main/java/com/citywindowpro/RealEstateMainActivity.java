...
public class RealEstateMainActivity extends AppCompatActivity implements ViewPager.OnPageChangeListener {
    RecyclerView rv_realestste_main;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        rv_realestste_main = (RecyclerView) findViewById(R.id.rv_realestste_main);
    ...
    public void subcategoryload() {
        if (isNetworkAvailable()) {
            ...
            RequestParams params = new RequestParams();
            params.add("cat_id", Constants.cat_id);
            client.post(Constants.liveuri + "sel_sub_child.php", params, new TextHttpResponseHandler() {
                @Override
                public void onFailure(int statusCode, Header[] headers, String responseString, Throwable throwable) {
                    dialog.dismiss();
                }
                ...
                public void onSuccess(int statusCode, Header[] headers, String responseString) {
                    if (responseString != null) {
                        ...
                        rv_realestste_main.setLayoutManager(gridLayoutManager); // set LayoutManager to RecyclerView
                        //  call the constructor of FashionistaMainAdapter to send the reference and data to Adapter
                        FiveMainProductAdapter customAdapter = new FiveMainProductAdapter(RealEstateMainActivity.this, listuser);
                        rv_realestste_main.setAdapter(customAdapter);
                        rv_realestste_main.addOnItemTouchListener(new RecyclerItemClickListener(RealEstateMainActivity.this, new RecyclerItemClickListener.OnItemClickListener() {
                            @Override
                            public void onItemClick(View view, int position) {
                                Constants.sub_id = listuser.get(position).getSubid();
                                startActivity(new Intent(getApplicationContext(), RealEstateSubcategory1Activity.class));
                            }
                        }));
                    }
                }
            ...
            Toast.makeText(RealEstateMainActivity.this, "No Internet connection", Toast.LENGTH_SHORT).show();
        }
    }
